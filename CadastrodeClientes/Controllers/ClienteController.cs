...
public class ClienteController
{
    ...
    public static void GerenciarClientes()
    {
        ...
        while (true)
        {
            ClienteValidator clientValidator = new ClienteValidator();
            ...
            Console.WriteLine("5 - Atualizar Cliente por Id");
            ...
            if (opcao == "1")
            {
                ...
                if(StatusDoCliente.Bloqueado == status)
                {
                    clientValidator.ValidarStatusDoCliente(cliente, status);
                    Console.WriteLine("Cliente bloqueado, não é possível realizar o cadastro!");
                    return;
                }

                if  (StatusDoCliente.Inativo == status)
                {
                    clientValidator.ValidarStatusDoCliente(cliente, status);
                    Console.WriteLine("Cliente inativo, não é possível realizar o cadastro!");
                    return;
                }
            ...
            if (opcao == "2")
            {
                ...
                {
                    Console.WriteLine($"Cliente:\n ID: {Cliente.Id} NOME: {Cliente.Nome}, EMAIL: {Cliente.Email}, CPF: s{Cliente.Cpf}");
                }
            }
            ...
            if (opcao == "5")
            {
                Console.WriteLine("Informe o ID Do Cliente para Atualização");
                repo.AtualizarCliente(Guid.Parse(Console.ReadLine() ?? string.Empty), new Cliente(createClienteDto.Nome, createClienteDto.Email, createClienteDto.Cpf));
                Console.WriteLine("Informe o Nome do Cliente:");
                createClienteDto.Nome = Console.ReadLine() ?? string.Empty;
